...
function Shell(){
  ...
  const links=[['/dashboard',session.role==='COMMANDER'?'상황판':'대시보드',session.role==='COMMANDER'?Radar:LayoutDashboard],...(session.role!=='COMMANDER'?[['/builder','워크플로우 빌더',Network],['/agents','워크플로우 레지스트리',Bot],['/react-agent-builder','에이전트 빌더',Bot],['/react-agents','에이전트 레지스트리',Network],['/executions','실행 모니터링',Activity],...(session.role==='ADMIN'?[]:[['/situation','상황판',Radar]])] as any:[]),['/manual','사용 매뉴얼',BookOpen],...(session.role==='ANALYST'?[['/sensor','센서 입력',Radio]] as any:[]),...(session.role==='ADMIN'?[['/leave-request','휴가 신청 입력',CalendarDays]] as any:[])];
