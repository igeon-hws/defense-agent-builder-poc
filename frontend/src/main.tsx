...
import {Activity, Bell, BookOpen, Bot, CalendarDays, Check, ChevronRight, ClipboardCheck, FileText, LayoutDashboard, LogOut, Network, Play, Radar, Radio, Save, Shield, SlidersHorizontal, Trash2, Users, X} from 'lucide-react';
import './styles.css';
import './light.css';
import './sensor.css';
import './agent.css';
...
const API='/api';
type Role='ANALYST'|'STAFF'|'COMMANDER'|'ADMIN';
...
type ReactAgentSummary={id:string;name:string;description:string;role:Role;lifecycle:string;version:number;definition:any;system_default?:boolean;default_kind?:'GENERAL'|'MISSION'};
type Execution={id:string;agent_id:string;agent_name:string;role:Role;area:string;version:number;status:string;created_at:string;trigger_type?:string;trigger?:any;source_report_id?:string;child_execution_id?:string;trace?:any[];approval?:any;reports?:any[];snapshot?:any;intranet_registration?:any};
...
const roleName={ANALYST:'파주지역 분석관',STAFF:'정보·작전 참모',COMMANDER:'지휘관',ADMIN:'행정병'};
const userName:Record<string,string>={'analyst.a12':'김분석 분석관','staff.ops':'이작전 참모','commander.demo':'박지휘 지휘관','admin.hr01':'최행정 행정병'};
const statusName:Record<string,string>={PUBLISHED:'게시됨',DRAFT:'초안',COMPLETED:'완료',REJECTED:'반려',RUNNING:'실행 중',WAITING_FOR_ANALYST_APPROVAL:'분석관 승인 대기',WAITING_FOR_STAFF_APPROVAL:'참모 승인 대기',WAITING_FOR_ADMIN_APPROVAL:'행정병 승인 대기',REGISTERED:'인트라넷 등록 완료',SUCCEEDED:'완료',WAITING:'승인 대기',FAILED:'실패'};

function App(){const [session,setSession]=useState<Session|null>(null);async function login(role:Role){setSession(await api('/session',{method:'POST',body:JSON.stringify({role})}))}return <SessionContext.Provider value={{session,login,logout:()=>setSession(null)}}><BrowserRouter><Routes><Route path="/" element={<Navigate to="/login" replace/>}/><Route path="/login" element={<Login/>}/><Route path="/*" element={session?<Shell/>:<Navigate to="/login"/>}/></Routes></BrowserRouter></SessionContext.Provider>}

function Login(){const {login}=useContext(SessionContext);const nav=useNavigate();const [role,setRole]=useState<Role>('ADMIN');const description=(r:Role)=>r==='ANALYST'?'파주시 감시 이벤트 분석 및 지역 보고':r==='STAFF'?'다지역 정보 종합 및 지휘관 보고':r==='COMMANDER'?'승인된 최종 상황 보고 열람':'휴가·외출·외박 인사행정 처리';return <main className="login"><div className="loginBrand"><div className="emblem"><Shield size={44}/></div><div><span>DEFENSE AI PLATFORM</span><h1>Workflow Builder</h1><p>지능형 국방 의사결정 워크플로</p></div></div><section className="loginCard"><div className="mock">DEMO · MOCK LOGIN</div><h2>임무 역할을 선택하세요</h2><p>선택한 역할에 맞춰 워크플로우와 데이터 접근 범위가 설정됩니다.</p><div className="roleCards">{(['ADMIN','ANALYST','STAFF','COMMANDER'] as Role[]).map((r)=><button key={r} className={role===r?'role active':'role'} onClick={()=>setRole(r)}><span className="roleIcon">{r==='ANALYST'?<Radar/>:r==='STAFF'?<Network/>:r==='COMMANDER'?<Users/>:<ClipboardCheck/>}</span><span><b>{roleName[r]}</b><small>{description(r)}</small></span><span className="radio">{role===r&&<Check size={14}/>}</span></button>)}</div><button className="primary loginBtn" onClick={async()=>{await login(role);nav('/dashboard')}}>Workflow Builder 시작 <ChevronRight size={18}/></button><small className="security">훈련용 가상 데이터 · 모든 활동은 감사 기록에 저장됩니다</small></section></main>}

function BuilderEntry(){const {session}=useContext(SessionContext);if(session?.role==='COMMANDER')return <Navigate to="/dashboard" replace/>;const target=session?.role==='STAFF'?'staff-synthesis':session?.role==='ADMIN'?'admin-leave-registration':'analyst-a12';return <Navigate to={'/agents/'+target+'/builder'} replace/>}
...
function Shell(){
  ...
  const links=[['/dashboard',session.role==='COMMANDER'?'상황판':'대시보드',session.role==='COMMANDER'?Radar:LayoutDashboard],...(session.role!=='COMMANDER'?[['/builder','워크플로우 빌더',Network],['/agents','워크플로우 레지스트리',Bot],['/react-agent-builder','에이전트 빌더',Bot],['/react-agents','에이전트 레지스트리',Network],['/executions','실행 모니터링',Activity],...(session.role==='ADMIN'?[]:[['/situation','상황판',Radar]])] as any:[['/react-agents','기본 에이전트',Bot]] as any),['/manual','사용 매뉴얼',BookOpen]];
...
function Dashboard(){const {session}=useContext(SessionContext);const nav=useNavigate();const [data,setData]=useState<any>();const [workflows,setWorkflows]=useState<Agent[]>([]);const [reactAgents,setReactAgents]=useState<ReactAgentSummary[]>([]);useEffect(()=>{if(!session)return;Promise.all([api('/dashboard?role='+session.role),api('/agents?role='+session.role,{headers:{'X-Demo-Role':session.role}}),api('/react-agents?role='+session.role,{headers:{'X-Demo-Role':session.role}})]).then(([d,w,r])=>{setData(d);setWorkflows(w);setReactAgents(r)})},[session]);if(!session)return null;if(session.role==='COMMANDER')return <SituationMap commanderHome/>;const description=(a:Agent)=>a.role==='ANALYST'?'센서 이벤트를 분석하고 지역 위협 보고서를 생성합니다.':a.role==='STAFF'?'승인된 지역 보고를 종합하여 지휘관 상황 보고를 생성합니다.':'휴가 신청의 잔여 일수와 부대 일정을 요약하고 승인 후 인트라넷에 등록합니다.';const trigger=(a:Agent)=>a.role==='ANALYST'?'Sensor Event':a.role==='STAFF'?'Approved Report':'Leave Request';const input=session.role==='ANALYST'?{title:'센서 입력',path:'/sensor',icon:<Radio/>}:session.role==='ADMIN'?{title:'휴가 입력',path:'/leave-request',icon:<CalendarDays/>}:null;return <><PageHead eyebrow={`${roleName[session.role]} · ${session.area}`} title="업무 자동화 대시보드"><div className="headButtons">{input&&<button className="secondary quietInputButton" onClick={()=>nav(input.path)}>{input.icon} {input.title}</button>}<button className="secondary" onClick={()=>nav('/react-agents')}><Bot size={17}/> 에이전트 열기</button></div></PageHead>{data?.pending_count>0&&<ApprovalAlert count={data.pending_count}/>}<div className="metrics"><Metric label="운영 워크플로우" value={workflows.length} icon={<Network/>}/><Metric label="사용 가능 에이전트" value={reactAgents.length} icon={<Bot/>}/><Metric label="활성 실행" value={data?.active_count||0} icon={<Activity/>}/><Metric label="승인 대기" value={data?.pending_count||0} icon={<ClipboardCheck/>}/></div><div className="dashboardAutomationGrid"><section className="panel"><div className="panelTitle"><h2>내 워크플로우</h2><button className="textBtn" onClick={()=>nav('/agents')}>전체 보기 <ChevronRight size={15}/></button></div>{workflows.length?workflows.map(a=><article className="agentCard" key={a.id}><div className="agentMark"><Network/></div><div className="grow"><div><Badge value={a.lifecycle}/><span className="muted">v{a.version} · {a.area}</span></div><h3>{a.name}</h3><p>{description(a)}</p><div className="chips"><span>Trigger</span><b>{trigger(a)}</b></div></div><button className="secondary" onClick={()=>nav('/agents/'+a.id+'/builder')}>빌더 열기</button></article>):<Empty text="사용 가능한 워크플로우가 없습니다."/>}</section><section className="panel"><div className="panelTitle"><h2>내 에이전트</h2><button className="textBtn" onClick={()=>nav('/react-agents')}>전체 보기 <ChevronRight size={15}/></button></div>{reactAgents.length?reactAgents.slice(0,3).map(a=><article className="agentCard reactDashboardCard" key={a.id}><div className="agentMark"><Bot/></div><div className="grow"><div><Badge value={a.lifecycle}/>{a.system_default&&<span className="systemDefault">{a.default_kind==='MISSION'?'MISSION':'GENERAL'}</span>}</div><h3>{a.name}</h3><p>{a.description}</p><div className="chips"><span>Tools</span><b>{a.definition.tools.length}개 연결</b></div></div><button className="primary" onClick={()=>nav('/react-agents/'+a.id+'/chat')}>실행</button></article>):<Empty text="사용 가능한 에이전트가 없습니다."/>}</section></div><section className="panel dashboardNotifications"><div className="panelTitle"><h2>승인 및 알림</h2><Bell size={18}/></div>{data?.notifications?.length?data.notifications.map((n:any)=><button className={'notification '+(n.read_at?'read':'unread')} key={n.id} onClick={async()=>{const wasUnread=!n.read_at;const readAt=await markNotificationRead(n,session.role);if(wasUnread)setData((current:any)=>({...current,unread_count:Math.max(0,(current.unread_count||0)-1),notifications:current.notifications.map((item:any)=>item.id===n.id?{...item,read_at:readAt}:item)}));nav('/executions/'+n.execution_id)}}><span className="pulse"/><div><b>{n.title}</b><p>{n.body}</p><small>{n.execution_id} · {n.read_at?'확인함':'새 알림'}</small></div><ChevronRight/></button>):<Empty text="새로운 알림이 없습니다."/>}</section></>}
...
const NODE_IO:Record<string,{input:string[];output:string[]}>={
  ...
  leave_request:{input:['휴가 신청서'],output:['event']},leave_balance:{input:['service_number'],output:['remaining_days']},
  unit_events:{input:['start_date','end_date'],output:['related_unit_events']},leave_summary:{input:['event','remaining_days','related_unit_events'],output:['eligible','summary','conflicts','draft']},
  intranet_register:{input:['draft','approval_decision'],output:['registration_id','status']},
};
...
function AgentRegistry(){
  ...
  async function create(){
    ...
    try{
      const result=await api('/agents',{method:'POST',headers:{'X-Demo-Role':session.role},body:JSON.stringify({
        ...form,role:session.role,area:session.role==='ANALYST'?'경기도 파주시':session.role==='ADMIN'?'제1행정부대':'접경지역 전체'
      })});
  ...
  return <>
    ...
    {creating&&<div className="modalBackdrop" onClick={()=>setCreating(false)}><article className="createModal" onClick={e=>e.stopPropagation()}><button className="close" onClick={()=>setCreating(false)}><X/></button><span>NEW WORKFLOW</span><h2>새 워크플로우 만들기</h2><p>빈 캔버스에서 직접 구성하거나 역할별 기본 템플릿으로 시작합니다.</p><label>워크플로우 이름<input autoFocus value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder={session?.role==='ADMIN'?'예: 정기 휴가 검토 워크플로우':'예: 파주 북부 감시 워크플로우'}/></label><label>설명<textarea value={form.description} onChange={e=>setForm({...form,description:e.target.value})} placeholder="수행할 임무를 입력하세요."/></label><label>시작 방식<select value={form.template} onChange={e=>setForm({...form,template:e.target.value})}><option value="BLANK">빈 캔버스</option><option value={session?.role}>{session?.role==='ANALYST'?'분석관':session?.role==='STAFF'?'참모':'행정병'} 기본 템플릿</option></select></label><div className="createScope"><span>역할</span><b>{session&&roleName[session.role]}</b><span>범위</span><b>{session?.area}</b></div>{error&&<p className="error">{error}</p>}<div className="createActions"><button className="secondary" onClick={()=>setCreating(false)}>취소</button><button className="primary" onClick={create}>초안 만들기 <ChevronRight/></button></div></article></div>}
  </>
}
...
function SituationMap({commanderHome=false}:{commanderHome?:boolean}){
  ...
  const [commanderAgents,setCommanderAgents]=useState<ReactAgentSummary[]>([]);
  ...
  useEffect(()=>{if(session)Promise.all([api('/reports?role='+session.role),api('/situation-board?role='+session.role),...(session.role==='COMMANDER'?[api('/react-agents?role=COMMANDER',{headers:{'X-Demo-Role':'COMMANDER'}})]:[Promise.resolve([])])]).then(([r,a,agents])=>{setReports(r);setActivity(a);setCommanderAgents(agents)})},[session]);
  ...
  const commanderReports=reports.filter(report=>report.kind==='COMMANDER');
  const latestCommanderReport=commanderReports[0];
  const staffSourceReports=(latestCommanderReport?.source_report_ids||[]).map((sourceId:string)=>reports.find(report=>report.id===sourceId)).filter(Boolean);
  return <>
    <PageHead eyebrow={commanderHome?"지휘관 공통 상황도":"실제 지도 기반 상황 인식"} title="접경지역 상황판"><span className="live"><span className="online"/> {session?.role==='COMMANDER'?'참모 승인 보고 기준':'실시간 이벤트·승인 보고'}</span></PageHead>
    <div className="mapLayout">
      ...
      <aside className="areaList">
        <h2>감시 지역</h2>
        {areas.map(area=>{const report=session?.role==='COMMANDER'?staffSourceReports.find((x:any)=>x.area===area):reports.find(x=>x.area===area);return <button key={area} className={selectedArea===area?'areaItem selected':'areaItem'} onClick={()=>setSelectedArea(area)}><div><b>{area}</b><small>{report?.title||(session?.role==='COMMANDER'?'최신 참모 보고에 포함되지 않음':'승인 보고 없음')}</small></div><Badge value={report?.threat||'LOW'}/></button>})}
      </aside>
    </div>
    {session?.role==='COMMANDER'&&<div className="commanderFeed"><section className="panel"><div className="panelTitle"><h2>참모 종합 판단</h2><span>{commanderReports.length}건</span></div>{latestCommanderReport?<button className="staffBriefing" onClick={()=>setSelectedReport(latestCommanderReport)}><div className="staffBriefingHead"><div><small>최신 승인 보고</small><b>{latestCommanderReport.title}</b></div><Badge value={latestCommanderReport.threat}/></div><p>{latestCommanderReport.content}</p><div className="staffBriefingMeta"><span>보고 시각 <b>{new Date(latestCommanderReport.approved_at).toLocaleString('ko')}</b></span><span>참조 지역 <b>{staffSourceReports.length}개</b></span><span>승인 참모 <b>{latestCommanderReport.approved_by}</b></span></div><strong>종합 보고서 열기 <ChevronRight/></strong></button>:<Empty text="참모가 승인해 전달한 종합 보고서가 없습니다."/>}</section>
    ...
    {commanderHome&&<section className="panel commanderAgentPanel"><div className="panelTitle"><h2>지휘관 에이전트</h2><button className="textBtn" onClick={()=>nav('/react-agents')}>전체 보기 <ChevronRight size={15}/></button></div><div className="commanderAgentGrid">{commanderAgents.map(agent=><article className="agentCard reactDashboardCard" key={agent.id}><div className="agentMark"><Bot/></div><div className="grow"><div><Badge value={agent.lifecycle}/>{agent.system_default&&<span className="systemDefault">{agent.default_kind==='MISSION'?'MISSION':'GENERAL'}</span>}</div><h3>{agent.name}</h3><p>{agent.description}</p><div className="chips"><span>Tools</span><b>{agent.definition.tools.length}개 연결</b></div></div><button className="primary" onClick={()=>nav('/react-agents/'+agent.id+'/chat')}>실행</button></article>)}</div></section>}
...
function NodeSettings({node,models,onChange,onDelete}:{node:any;models:any[];onChange:(node:any)=>void;onDelete:()=>void}){
  ...
  return <><div className="configType"><b>{GROUP_META[node.data.group]?.name||node.data.group}</b><span>{GROUP_META[node.data.group]?.description}</span></div>
    {connector&&<div className="connectorSummary"><span>{connector.category==='DATA'?'연결 데이터':'외부 시스템'}</span><b>{connector.name}</b><em>{connector.mode} 연동</em></div>}
    ...
    {capability==='leave_request'&&<div className="info">정기 휴가 신청서가 접수되면 신청자·기간·신청 일수를 전달합니다.</div>}
    {(capability==='approval'||capability==='send'||capability==='report_trigger'||capability==='reports'||capability==='leave_balance'||capability==='unit_events'||capability==='intranet_register')&&<div className="info">이 노드는 워크플로우 역할과 연결 시스템에 맞춰 자동 설정됩니다.</div>}
...
function LeaveRequestInput(){
  const {session}=useContext(SessionContext);
  const nav=useNavigate();
  const [form,setForm]=useState({service_number:'23-12345678',member_name:'김민준',unit:'제1행정부대 본부중대',leave_type:'정기 휴가',start_date:'2026-09-18',end_date:'2026-09-20',requested_days:3});
  const [result,setResult]=useState<any>();
  const [sending,setSending]=useState(false);
  const [error,setError]=useState('');
  if(session?.role!=='ADMIN')return <Navigate to="/dashboard"/>;
  const set=(key:string,value:any)=>setForm(current=>({...current,[key]:value}));
  async function submit(){setSending(true);setError('');try{setResult(await api('/leave-requests',{method:'POST',headers:{'X-Demo-Role':'ADMIN'},body:JSON.stringify(form)}))}catch(e:any){setError(e.message)}finally{setSending(false)}}
  return <><PageHead eyebrow="DEMO PERSONNEL INPUT" title="정기 휴가 신청 접수"><span className="muted">부대 인트라넷 신청을 대신하는 데모 입력기</span></PageHead>
    <div className="sensorLayout"><section className="panel sensorForm"><div className="sensorIdentity"><CalendarDays/><div><small>병역관리 및 인사행정</small><h2>정기 휴가 신청서</h2><span><i className="online"/> 제1행정부대 · 인사행정 시스템 정상</span></div></div>
      <div className="leaveFields"><label>군번<input value={form.service_number} onChange={e=>set('service_number',e.target.value)}/></label><label>성명<input value={form.member_name} onChange={e=>set('member_name',e.target.value)}/></label></div>
      <label>소속<input value={form.unit} onChange={e=>set('unit',e.target.value)}/></label>
      <label>휴가 종류<select value={form.leave_type} onChange={e=>set('leave_type',e.target.value)}><option>정기 휴가</option><option>포상 휴가</option><option>청원 휴가</option></select></label>
      <div className="leaveFields"><label>시작일<input type="date" value={form.start_date} onChange={e=>set('start_date',e.target.value)}/></label><label>종료일<input type="date" value={form.end_date} onChange={e=>set('end_date',e.target.value)}/></label></div>
      <label>신청 일수<input type="number" min="1" max="15" value={form.requested_days} onChange={e=>set('requested_days',+e.target.value)}/></label>
      <button className="primary sensorSend" onClick={submit} disabled={sending}><Play/>{sending?' 신청 처리 중…':'휴가 신청 접수'}</button>{error&&<p className="error">{error}</p>}
    </section><aside className="panel sensorPreview"><h2>워크플로우 입력 데이터</h2><pre>{JSON.stringify(form,null,2)}</pre>
      <div className="scaleHint"><b>자동 조회 항목</b><p>잔여 휴가 12일과 신청 기간의 당직·복귀 점검 일정을 조회한 뒤 AI가 승인용 요약을 작성합니다.</p></div>
      {result&&<div className="sensorResult"><Check/><div><small>신청이 접수되어 HITL 승인 단계로 이동했습니다.</small><b>{result.leave_request_id} · 잔여 {result.remaining_days}일</b><Badge value={result.status}/></div><button className="secondary" onClick={()=>nav('/executions/'+result.execution_id)}>검토 열기 <ChevronRight/></button></div>}
    </aside></div></>;
}
...
const ROLE_GUIDE={
  ...
  ADMIN:{title:'행정병',summary:'휴가 신청과 외출·외박 현황 등 병역관리 및 인사행정 업무를 처리합니다.',permissions:['본인 소유 인사행정 워크플로우 생성·편집·게시·삭제','인사행정 ReAct 에이전트 생성·도구 연결·실행','휴가 신청 데모 입력','휴가 검토 승인 및 모의 인트라넷 등록'],nodes:['정기 휴가 신청 접수','잔여 휴가 조회','부대 일정 조회','휴가 신청 요약 생성','행정병 검토·승인','부대 인트라넷 등록']},
} as const;
...
function Manual(){
  ...
  const steps=session.role==='ANALYST'
    ?['워크플로우 레지스트리에서 새 워크플로우를 만들거나 기본 워크플로우를 엽니다.','노드를 배치·연결하고 조건과 AI 프롬프트를 설정한 뒤 저장, 검증, 게시합니다.','에이전트 빌더의 연동 카탈로그에서 작전 데이터와 외부 시스템을 선택하고 게시한 뒤 레지스트리에서 채팅을 실행합니다.','센서 입력에서 탐지 규모와 신뢰도를 정해 이벤트를 전송합니다.','실행 모니터링에서 LLM 분석을 확인하고 지역 보고서를 승인합니다.']
    :session.role==='STAFF'
    ?['워크플로우 레지스트리에서 종합 워크플로우의 노드와 프롬프트를 설정하고 게시합니다.','에이전트 빌더의 연동 카탈로그에서 보고서·지역 데이터와 지휘 보고 체계를 선택하고 모델을 설정합니다.','분석관이 지역 보고서를 승인하면 종합 워크플로우가 자동 실행됩니다.','실행 모니터링에서 지역별 근거와 LLM 종합 결과를 확인합니다.','지휘관 보고서 초안을 검토하고 승인합니다.']
    :session.role==='ADMIN'
    ?['휴가 신청 입력에서 정기 휴가 신청서를 접수합니다.','워크플로우가 잔여 휴가와 관련 부대 일정을 조회하고 AI 승인 요약을 생성합니다.','에이전트 빌더에서 인사 데이터·규정과 부대 인트라넷 연동을 선택할 수 있습니다.','실행 모니터링에서 신청 내용과 근거를 확인한 뒤 승인하거나 반려합니다.','에이전트 채팅에서 “이번 주차 외출/외박 현황 보고 작성해줘”를 실행합니다.']
    :['상황판에서 파주·연천·철원 지역의 승인 보고를 지도와 함께 확인합니다.','기본 에이전트에서 추천 질문을 선택해 최신 참모 종합판단과 근거를 조회합니다.','최종 지휘관 보고서의 승인자와 원본 보고 계보를 확인합니다.'];
