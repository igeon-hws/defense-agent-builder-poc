...
function NotificationBell({session}:{session:Session}){
  ...
  return <button className={'iconBtn notificationBell '+(count?'hasAlert':'')} title={count?'승인 대기 '+count+'건':'새 승인 요청 없음'} onClick={()=>nav(session.role==='COMMANDER'?'/dashboard':'/executions')}><Bell size={18}/>{count>0&&<b>{count}</b>}</button>
}
...
function SituationMap({commanderHome=false}:{commanderHome?:boolean}){
  ...
  return <>
    ...
    <section className="panel"><div className="panelTitle"><h2>보고서 도착 알림</h2><span>{reportNotifications.length}건</span></div>{reportNotifications.length?reportNotifications.map(({note,report}:any)=><button className="feedItem reportArrival" key={note.id} onClick={()=>setSelectedReport(report)}><Bell/><div><b>{note.title}</b><p>{note.body}</p><small>{new Date(note.created_at).toLocaleString('ko')}</small></div><span>보고서 열기 <ChevronRight/></span></button>):<Empty text="새로 전달된 지휘관 보고서가 없습니다."/>}</section></div>}
