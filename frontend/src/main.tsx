...
function Dashboard(){const {session}=useContext(SessionContext);const nav=useNavigate();const [data,setData]=useState<any>();const [agents,setAgents]=useState<Agent[]>([]);const [execs,setExecs]=useState<Execution[]>([]);useEffect(()=>{if(!session)return;Promise.all([api('/dashboard?role='+session.role),api('/agents?role='+session.role,{headers:{'X-Demo-Role':session.role}}),api('/executions?role='+session.role)]).then(([d,a,e])=>{setData(d);setAgents(a);setExecs(e)})},[session]);if(!session)return null;if(session.role==='COMMANDER')return <SituationMap commanderHome/>;return <><PageHead eyebrow={`${roleName[session.role]} · ${session.area}`} title={false?'지휘 상황 개요':'에이전트 운영 대시보드'}>{true&&agents[0]&&<button className="primary" onClick={()=>nav('/agents/'+agents[0].id+'/builder')}><Bot size={17}/> 에이전트 열기</button>}</PageHead>{data?.pending_count>0&&<ApprovalAlert count={data.pending_count}/>}<div className="metrics"><Metric label="운영 에이전트" value={data?.agent_count||0} icon={<Bot/>}/><Metric label="활성 실행" value={data?.active_count||0} icon={<Activity/>}/><Metric label="승인 대기" value={data?.pending_count||0} icon={<ClipboardCheck/>}/><Metric label="시스템 상태" value="정상" icon={<Shield/>}/></div><div className="dashboardGrid"><section className="panel"><div className="panelTitle"><h2>{false?'최근 완료 실행':'내 에이전트'}</h2><button className="textBtn" onClick={()=>nav(false?'/situation':'/agents')}>전체 보기 <ChevronRight size={15}/></button></div>{true?agents.map(a=><article className="agentCard" key={a.id}><div className="agentMark"><Network/></div><div className="grow"><div><Badge value={a.lifecycle}/><span className="muted">v{a.version} · {a.area}</span></div><h3>{a.name}</h3><p>{a.role==='ANALYST'?'센서 이벤트를 분석하고 지역 위협 보고서를 생성합니다.':'승인된 지역 보고를 종합하여 지휘관 상황 보고를 생성합니다.'}</p><div className="chips"><span>Trigger</span><b>{a.role==='ANALYST'?'Sensor Event':'Approved Report'}</b></div></div><button className="secondary" onClick={()=>nav('/agents/'+a.id+'/builder')}>빌더 열기</button></article>):execs.slice(0,4).map(e=><RunRow key={e.id} e={e}/>)}</section><section className="panel"><div className="panelTitle"><h2>승인 및 알림</h2><Bell size={18}/></div>{data?.notifications?.length?data.notifications.map((n:any)=><button className="notification" key={n.id} onClick={()=>nav('/executions/'+n.execution_id)}><span className="pulse"/><div><b>{n.title}</b><p>{n.body}</p><small>{n.execution_id}</small></div><ChevronRight/></button>):<Empty text="새로운 알림이 없습니다."/>}</section></div></>}
...
function NotificationBell({session}:{session:Session}){
  ...
  useEffect(()=>{const load=()=>api('/dashboard?role='+session.role).then(x=>setCount(x.pending_count||0)).catch(()=>{});load();const timer=setInterval(load,5000);return()=>clearInterval(timer)},[session.role]);
  return <button className={'iconBtn notificationBell '+(count?'hasAlert':'')} title={count?'승인 대기 '+count+'건':'새 승인 요청 없음'} onClick={()=>nav(session.role==='COMMANDER'?'/situation':'/executions')}><Bell size={18}/>{count>0&&<b>{count}</b>}</button>
}
...
function AgentRegistry(){
  ...
  async function create(){
    if(!session||!form.name.trim())return setError('에이전트 이름을 입력하세요.');
    try{
...
function SituationMap({commanderHome=false}:{commanderHome?:boolean}){
  ...
  return <>
    ...
    <section className="panel"><div className="panelTitle"><h2>지휘 알림</h2><span>{activity.notifications.length}건</span></div>{activity.notifications.length?activity.notifications.map((note:any)=><article className="feedItem" key={note.id}><Bell/><div><b>{note.title}</b><p>{note.body}</p><small>{new Date(note.created_at).toLocaleString('ko')}</small></div></article>):<Empty text="새로운 알림이 없습니다."/>}</section></div>}
    <section className="panel"><div className="panelTitle"><h2>승인된 지역 보고서</h2><span>{reports.length}건</span></div><div className="reportGrid">{reports.map(r=><button className="reportCard" key={r.id} onClick={()=>setSelectedReport(r)}><div><FileText/><Badge value={r.threat}/></div><h3>{r.title}</h3><p>{r.content}</p><small>{r.id} · {r.approved_by}{r.fixture&&<b> 데모 기본자료</b>}</small></button>)}</div></section>
...
function NodeSettings({node,models,onChange,onDelete}:{node:any;models:any[];onChange:(node:any)=>void;onDelete:()=>void}){
  ...
  return <><div className="configType"><b>{GROUP_META[node.data.group]?.name||node.data.group}</b><span>{GROUP_META[node.data.group]?.description}</span></div>
    ...
    {(capability==='approval'||capability==='send'||capability==='report_trigger'||capability==='reports')&&<div className="info">이 노드는 에이전트 역할과 보고서 유형에 맞춰 자동 설정됩니다.</div>}
...
const ROLE_GUIDE={
  ANALYST:{title:'파주지역 분석관',summary:'파주시 센서 이벤트를 분석하고 지역 보고서를 검토·승인합니다.',permissions:['본인 소유 분석 에이전트 생성·편집·게시·삭제','파주시 센서 이벤트 실행','분석관 승인 요청 처리'],nodes:['감시 센서 이벤트','이벤트 조건 확인','작전 정보 조회','위협 분석·초안 생성','분석관 검토·승인','지역 보고서 발행']},
  STAFF:{title:'정보·작전 참모',summary:'승인된 지역 보고서를 종합하고 지휘관 상황보고를 검토·승인합니다.',permissions:['본인 소유 종합 에이전트 생성·편집·게시·삭제','승인 지역보고 기반 자동 실행','참모 승인 요청 처리'],nodes:['승인 지역보고 접수','승인 지역보고 수집','접경지역 작전상황 조회','위협 종합·초안 생성','참모 검토·승인','지휘관 보고서 발행']},
  COMMANDER:{title:'지휘관',summary:'승인이 끝난 최종 상황보고와 접경지역 상황판을 열람합니다.',permissions:['완료된 지휘관 보고서 열람','접경지역 통합 상황판 열람','에이전트 편집 및 승인 권한 없음'],nodes:[]},
} as const;
...
function Manual(){
  ...
  const steps=session.role==='ANALYST'
    ?['에이전트 레지스트리에서 새 에이전트를 만들거나 기본 에이전트를 엽니다.','노드를 배치·연결하고 조건과 AI 프롬프트를 설정한 뒤 저장, 검증, 게시합니다.','센서 입력에서 탐지 규모와 신뢰도를 정해 이벤트를 전송합니다.','실행 모니터링에서 LLM 분석을 확인하고 지역 보고서를 승인합니다.']
    :session.role==='STAFF'
    ?['에이전트 레지스트리에서 종합 에이전트의 노드와 프롬프트를 설정하고 게시합니다.','분석관이 지역 보고서를 승인하면 종합 에이전트가 자동 실행됩니다.','실행 모니터링에서 지역별 근거와 LLM 종합 결과를 확인합니다.','지휘관 보고서 초안을 검토하고 승인합니다.']
