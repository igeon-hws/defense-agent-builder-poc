...
| AC-27 | 참모 레지스트리의 기본 에이전트에서 주간 위협 비교 추천 질문 실행 | `SYSTEM DEFAULT` 에이전트가 현재 참모 권한의 연동 전체를 사용하고, 추천 질문에 따라 최근 관측, 지난주 승인 보고 기준과 근거 종합 도구를 선택한다. 결과에는 이번 주·지난주 비교 수치와 근거 식별자가 포함된다. |
| AC-28 | 네 역할의 에이전트 레지스트리 진입 | 각 역할에 시스템 기본 에이전트가 정확히 하나 표시되고 권한 범위의 연동이 자동 연결된다. 기존 대표 유스케이스는 추천 질문으로 표시되며 지휘관 기본 에이전트에는 읽기 도구만 노출된다. |
