...
에이전트 빌더는 워크플로우 빌더와 별도 메뉴다. 분석관·참모·지휘관·행정병에게 역할별 범용 기본 에이전트와 임무 특화 기본 에이전트를 하나씩 제공한다. 범용형은 현재 역할의 전체 연동을, 임무 특화형은 목적별 도구 구성을 사용하며 둘 다 삭제·편집할 수 없다. 임무 특화형은 각각 징후 상관분석, 대응 우선순위, 지휘결심 검토, 근무편성 점검을 수행하고 에이전트별 추천 질문을 제공한다. 사용자가 만든 커스텀 에이전트는 역할별 연동 카탈로그에서 데이터와 외부 시스템을 선택하고 모델, 프롬프트와 최대 반복 5~8회를 저장·게시할 수 있다. 각 카탈로그 항목은 `MOCK`, `DATA`/`SYSTEM`, `READ`/`SEARCH`/`WRITE`를 표시한다.
