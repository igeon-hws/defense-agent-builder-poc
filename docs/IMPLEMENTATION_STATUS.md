...
에이전트 빌더는 워크플로우 빌더와 별도 메뉴다. 역할별 연동 카탈로그에서 데이터와 외부 시스템을 선택하고 모델, 프롬프트와 최대 반복 5~8회를 저장·게시할 수 있다. 분석관·참모 카탈로그는 작전 관측 데이터, 승인 보고서, 지역 상황 정보와 지휘 보고 체계를 제공하고, 행정병 카탈로그는 인사행정 데이터, 부대 일정·규정과 부대 인트라넷을 제공한다. 각 항목은 `MOCK`, `DATA`/`SYSTEM`, `READ`/`SEARCH`/`WRITE`를 표시한다. 선택한 connector가 제공하는 도구만 런타임 허용 목록으로 계산된다. 기본 `파주시 이상징후 조사 에이전트`, 참모용 `주간 위협 수준 비교 에이전트`, `주간 외출·외박 현황 보고 에이전트`가 게시 상태로 생성된다.
